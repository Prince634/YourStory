...
const UploadImage = ({addImage, onAdd, addIcons, demo})=>{
	...
	const uploadImage = useCallback((event)=>{
		...
		reader.onload = function() {
		  ...
		  onAdd({src:`http://localhost:4005/${files[0].name}`/*reader.result*fileInfo: files[0],*/ ,value: files[0].name, tag: 'img', isIcon: addIcons, isDragable: addIcons, fixedImg: !addIcons});
		};
	...
	return(
			<div className={uploadWidget}>
				<div className={uploadBtn}>
					...
					<label className="btn" for={demo}>Upload Image
						<input id={demo} type="file" accept="image/*;capture=camera" style={{display: 'none'}} onChange={(e)=>uploadImage(e)} />
					</label>
					{
